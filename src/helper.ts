...
export async function getContractToken(
  address: string,
  decimals?: number,
  symbol?: string,
  name?: string
...
const basePairs: [Token, Token][] = lodash
  .flatMap(TRADE_TOKENS, (base): [Token, Token][] =>
    TRADE_TOKENS.map(otherBase => [base, otherBase])
  )
