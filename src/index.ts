...
const scheduleMonitor = async () => {
  ...
  monitor.on('liquidity.timer', async (amount, trade) => {
    ...
    logger.trace(
      `swap.price.update: ${task.wallet.outputAmount} / percent:${swapper
        .getPrc(task.swap.currentPrice)
        .toFixed(5)} / [C=${task.swap.currentPrice},B=${task._buyedPrice}]`
    ); //当前价格
  ...
  monitor.on('wallet.update.output_token', async wallet => {
    if (task.wallet.outputAmount !== wallet.amount && task._buyedPrice) {
      logger.trace(
        `wallet.update.output_token: ${wallet.amount} / ${swapper
          .getPrc(task.swap.currentPrice)
          .toFixed(5)}`
      ); //当前价格
    }
...
export class Swapper {
  ...
  async init() {
    ...
    logger.info(
      `OutputToken loaded:${this.outputTokenAddress} / ${this.outputToken.symbol} / ${this.outputToken.decimals}`
    );
  ...
  async execSwap(amount: string, trade) {
    try {
      ...
      const encoded_tx = routerContract.methods[parameters.methodName](
        ...parameters.args
      ).encodeABI();
  ...
  printTrade(tag: string, amount, trade) {
    ...
    if (this.cached.route != old.route || this.cached.price != old.price) {
      logger.warn(
        `[${tag}]Route.stateChange: ${SwapRoutePrint(trade).join('->')} / Price:${
          info.executionPrice
        },Input:${info.inputAmount},Output:${info.outputAmount}`
      );
    }
    return info;
  }
  ...
  private async _doSell(amount, currentPrice) {
    try {
      ...
      logger.trace(
        `AutoSell->[${prc},${needSellType}]->BuyPrice:${task._buyedPrice}->CurrentPrice:${currentPrice},amount:${task.wallet.outputAmount}`
      );
